...
import { useForumChat } from "@/hooks/data/useForumChat";
import {
  Send,
  Users,
  AlertCircle,
...
interface ForumChatFeatureProps {
  forumId: string;
  forumName?: string;
}
...
export function ForumChatFeature({ forumId, forumName = "Forum" }: ForumChatFeatureProps) {
  ...
  const { messages, isConnected, isLoading, error, sendMessage } = useForumChat({
    forumId,
    autoJoin: true,
    fetchHistory: true,
  });
        ...
        <div className="flex items-end gap-3">
          <div className="flex-1">
            <textarea
              ...
              className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-500 dark:placeholder-slate-400 resize-none min-h-[50px] max-h-32 disabled:opacity-50 disabled:cursor-not-allowed"
              rows={1}
            />
          </div>
          <button
