...
export function useGroupChat({ groupId, autoJoin = true, fetchHistory = true }: UseGroupChatOptions) {
  ...
  // Initialize socket and join group
  useEffect(() => {
    if (!autoJoin || !session?.accessToken || !groupId) {
      return;
    }
    ...
    const setupChat = async () => {
      try {
        ...
        if (mounted) {
          ...
          const cleanup = setupGroupChatHandlers(groupId, {
            onMessage: (message) => {
              setMessages((prev) => {
                // Avoid duplicates
          ...
          // Join the group if socket is already connected
          ...
          if (fetchHistory) {
            try {
              const history = await ChatService.getMessages(groupId, { limit: 100 });
              setMessages(
                history.map((msg) => ({
                  ...msg,
  ...
  }, [groupId, autoJoin, session?.accessToken, fetchHistory]);
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        setError(null);
        const success = await sendSocketMessage(groupId, content.trim());
        if (!success) {
  ...
  }, [groupId, messages.length, isLoading]);
