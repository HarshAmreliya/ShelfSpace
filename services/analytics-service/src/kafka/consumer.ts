  ...
  await consumer.run({
    eachMessage: async ({ message, partition }) => {
      ...
      try {
        ...
        console.log(
          `[kafka:consumer][analytics-service] received ${event.type} | userId=${event.userId ?? "n/a"} | partition=${partition}`
        );
        ...
        console.log(
          `[kafka:consumer][analytics-service] projected ${event.type} | userId=${event.userId ?? "n/a"}`
        );
      } catch (error) {
