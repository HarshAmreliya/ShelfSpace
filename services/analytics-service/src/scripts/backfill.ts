...
const ANALYTICS_API_URL =
  process.env.ANALYTICS_API_URL || "http://localhost:3008/api/analytics";
const USER_LIBRARY_SERVICE_URL =
  process.env.USER_LIBRARY_SERVICE_URL || "http://localhost:3003";
const BOOK_SERVICE_URL =
  process.env.BOOK_SERVICE_URL || "http://localhost:3004";
...
async function backfillForUser(userId: string, accessToken: string) {
  ...
  const lists: Array<{ id: string; name: string; bookIds: string[] }> = listsResponse.data || [];
  const events: any[] = [];
