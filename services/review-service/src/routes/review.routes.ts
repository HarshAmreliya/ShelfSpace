...
router.post(
  ...
  isAuthenticated,
  async (
    req: Request<{}, Review, z.infer<typeof createReviewSchema>>,
    res: Response<Review | { error: string; details?: any }>
  ) => {
...
router.put(
  ...
  isAuthenticated,
  async (
    req: Request<{ id: string }, Review, z.infer<typeof updateReviewSchema>>,
    res: Response<Review | { error: string; details?: any }>
  ) => {
    const { id } = req.params;
...
router.delete(
  ...
  async (
    req: Request<{ id: string }>,
    res: Response<{ error: string } | void>
  ) => {
    const { id } = req.params;
    try {
      const review = await prisma.review.findUnique({ where: { id } });
